...
const fs = require('fs');
const blazecert = require('./blazecert');
...
async function createCA({ organization, countryCode, state, locality, validity, key, cert }) {
  ...
  let ca;
  try {
    ca = await blazecert.createCA({ organization, countryCode, state, locality, validityDays: validity });
  } catch (err) {
...
async function createCert({ domains, caKey, caCert, validity, key, cert }) {
  ...
  try {
    ...
    return console.error(`Unable to read \`${caKey}\`. Please run \`blazecert create-ca\` to create a new certificate authority.`);
  }
  ...
  try {
    ...
    return console.error(`Unable to read \`${caCert}\`. Please run \`blazecert create-ca\` to create a new certificate authority.`);
  }

  //Create the certificate
  let tls;
  try {
    tls = await blazecert.createCert({ domains, validityDays: validity, caKey: ca.key, caCert: ca.cert });
  } catch (err) {
    return console.error(`Failed to create the certificate. Error: ${err.message}`);
  }
